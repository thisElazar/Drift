...
void AAtmosphereController::UpdateCloudMaterial()
{
    ...
    if (TargetTerrain)
    {
        float WorldSize = TargetTerrain->TerrainWidth * TargetTerrain->TerrainScale;
...
void AAtmosphereController::Tick(float DeltaTime)
{
    ...
    // CRITICAL: Time progression should continue even if GPU compute is disabled
    // This ensures day/night cycle keeps running during resets and initialization
    if (bEnableHybridIntegration)
    {
        UpdateHybridIntegration(DeltaTime);
    }
    
    // GPU-dependent updates only run if compute is enabled
    if (!bUseGPUCompute)
    {
        return;
    }
...
void AAtmosphereController::ActivateVolumetricRendering()
{
    ...
    UE_LOG(LogTemp, Warning, TEXT("âœ“ Volumetric cloud material bound to post-process"));
    
    // === Disable debug plane ===
    if (CloudDebugPlane)
    {
        CloudDebugPlane->SetVisibility(false);
        UE_LOG(LogTemp, Warning, TEXT("âœ“ Debug plane hidden"));
    }
    
    if (AtmosphereDebugPlane)
    {
        AtmosphereDebugPlane->SetActorHiddenInGame(true);
        UE_LOG(LogTemp, Warning, TEXT("âœ“ Atmosphere debug plane hidden"));
    }
...
void AAtmosphereController::DispatchCloudRenderingCompute(float DeltaTime)
{
    ...
    float TerrainWorldSizeX = TargetTerrain->TerrainWidth * TargetTerrain->TerrainScale;
    float TerrainWorldSizeY = TargetTerrain->TerrainHeight * TargetTerrain->TerrainScale;
...
void AAtmosphereController::DebugPrintIntegrationStatus()
{
    ...
    UE_LOG(LogTemp, Warning, TEXT("Sun Angle: %.1fÂ°"), GetSunAngleDegrees());
    UE_LOG(LogTemp, Warning, TEXT("Is Daytime: %s"), IsDaytime() ? TEXT("YES") : TEXT("NO"));
    
    UE_LOG(LogTemp, Warning, TEXT("\nComponent Status:"));
    UE_LOG(LogTemp, Warning, TEXT("  DirectionalLight: %s"),
           DirectionalLight ? TEXT("âœ“ Connected") : TEXT("âœ— Missing"));
    UE_LOG(LogTemp, Warning, TEXT("  SkyAtmosphere: %s"),
           SkyAtmosphereComponent ? TEXT("âœ“ Connected") : TEXT("âœ— Missing"));
    UE_LOG(LogTemp, Warning, TEXT("  VolumetricCloud: %s"),
           VolumetricCloudComponent ? TEXT("âœ“ Connected") : TEXT("âœ— Missing"));
...
static FAutoConsoleCommand DebugHybridIntegrationCmd(
    ...
    FConsoleCommandDelegate::CreateLambda([]()
    {
        ...
        for (TActorIterator<AAtmosphereController> It(GWorld); It; ++It)
        {
            ...
            {
                UE_LOG(LogTemp, Warning, TEXT("  âœ“ DirectionalLight: %s"),
                ...
                {
                    UE_LOG(LogTemp, Error, TEXT("    âœ— Light Component is NULL!"));
                }
            ...
            {
                UE_LOG(LogTemp, Error, TEXT("  âœ— DirectionalLight: NOT FOUND"));
            ...
            {
                UE_LOG(LogTemp, Warning, TEXT("  âœ“ SkyAtmosphere: Found"));
            }
            else
            {
                UE_LOG(LogTemp, Warning, TEXT("  âœ— SkyAtmosphere: NOT FOUND"));
            }
            
            if (Atmo->VolumetricCloudComponent)
            {
                UE_LOG(LogTemp, Warning, TEXT("  âœ“ VolumetricCloud: Found"));
            }
            else
            {
                UE_LOG(LogTemp, Warning, TEXT("  âœ— VolumetricCloud: NOT FOUND"));
            }
            ...
            UE_LOG(LogTemp, Warning, TEXT("  Sun Angle: %.1fÂ°"),
...
void AAtmosphereController::ApplyLatitudePreset()
{
    ...
    // At equator (0Â°): sun reaches 90Â° overhead
    // At 23.5Â° (tropics): sun reaches 90Â° on solstice
    // At 45Â° (mid-latitude): sun reaches 68Â° at summer solstice
    // At 66.5Â° (arctic): sun reaches 47Â° at summer solstice
    
    float AbsLat = FMath::Abs(Latitude);
    
    // Summer solstice sun elevation = 90Â° - latitude + 23.5Â° (tilt)
    // We use this as the "typical" max elevation
    MaxSunElevation = FMath::Clamp(90.0f - AbsLat + 23.5f, 23.5f, 90.0f);
    
    UE_LOG(LogTemp, Warning, TEXT("Applied latitude preset: %.1fÂ° â†’ Max sun elevation: %.1fÂ°"),
...
void AAtmosphereController::FindAndCacheUEComponents()
{
        ...
        if (Light && IsValid(Light))
        {
            ...
            if (LightName.Contains(TEXT("Sun")) || LightName.Contains(TEXT("Directional")))
            {
                ...
                if (!bHasStoredOriginalLightSettings)
                {
                    ...
                    UE_LOG(LogTemp, Warning, TEXT("  âœ“ Stored original sun settings: Intensity=%.2f"),
                           OriginalLightIntensity);
                }
                
                UE_LOG(LogTemp, Warning, TEXT("  âœ“ Found Sun DirectionalLight: %s"), *LightName);
                break;
            }
        }
    }
    ...
    if (!DirectionalLight)
    {
        for (TActorIterator<ADirectionalLight> It(World); It; ++It)
        {
            ...
            if (DirectionalLight)
            {
                ...
                UE_LOG(LogTemp, Warning, TEXT("  âœ“ Found DirectionalLight (fallback): %s"),
    ...
    if (bEnableMoonLight)
    {
        for (TActorIterator<ADirectionalLight> It(World); It; ++It)
        {
            ...
            if (Light && IsValid(Light) && Light != DirectionalLight)
            {
                ...
                if (LightName.Contains(TEXT("Moon")) || LightName.Contains(TEXT("Night")))
                {
                    ...
                    UE_LOG(LogTemp, Warning, TEXT("  âœ“ Found MoonLight: %s"), *LightName);
                    break;
                }
            }
        }
        ...
        if (!MoonLight)
        {
            ...
            if (MoonLight)
            {
                ...
                UE_LOG(LogTemp, Warning, TEXT("  âœ“ Created MoonLight automatically"));
            }
        }
    }
    ...
    if (bEnableAmbientNightLight)
    {
        for (TActorIterator<ASkyLight> It(World); It; ++It)
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("  âœ“ Found SkyLight: %s"), *SkyLightActor->GetName());
            break;
        }
        ...
        if (!SkyLightActor)
        {
            ...
            if (SkyLightActor)
            {
                ...
                UE_LOG(LogTemp, Warning, TEXT("  âœ“ Created SkyLight automatically"));
            }
        }
    }
    ...
    for (TActorIterator<AActor> It(World); It; ++It)
    {
        ...
        if (Actor && Actor->GetName().Contains(TEXT("SkyAtmosphere")))
        {
            ...
            if (SkyAtmosphereComponent)
            {
                UE_LOG(LogTemp, Warning, TEXT("  âœ“ Found SkyAtmosphere: %s"), *Actor->GetName());
                break;
            }
        }
    }
    ...
    for (TActorIterator<AActor> It(World); It; ++It)
    {
        ...
        if (Actor && Actor->GetName().Contains(TEXT("VolumetricCloud")))
        {
            ...
            if (VolumetricCloudComponent)
            {
                UE_LOG(LogTemp, Warning, TEXT("  âœ“ Found VolumetricCloud: %s"), *Actor->GetName());
                break;
            }
        }
    }
...
void AAtmosphereController::UpdateDirectionalLight(float DeltaTime)
{
    ...
    if (bShowIntegrationDebug && DebugTimer > 0.5f)
    {
        GEngine->AddOnScreenDebugMessage(100, 0.5f, FColor::Yellow,
            FString::Printf(TEXT("Sun: %s | Elev: %.1fÂ° | Int: %.2f"),
...
void AAtmosphereController::UpdateMoonLight(float DeltaTime)
{
    ...
    if (bShowIntegrationDebug && MoonDebugTimer > 0.5f)
    {
        GEngine->AddOnScreenDebugMessage(101, 0.5f, FColor::Cyan,
            FString::Printf(TEXT("Moon: %s | Elev: %.1fÂ° | Bright: %.0f%%"),
