...
// ===== DRIFT VALIDATION MACROS =====
// Centralized validation patterns to eliminate code duplication

// Water system validation
#define DRIFT_VALIDATE_WATER_SYSTEM(WaterSystemPtr) \
...
#define DRIFT_VALIDATE_WATER_SYSTEM_RET(WaterSystemPtr, RetVal) \
    if (!WaterSystemPtr || !WaterSystemPtr->IsSystemReady()) \
    { \
        UE_LOG(LogTemp, VeryVerbose, TEXT("%s: Water system not ready"), *FString(__FUNCTION__)); \
        return RetVal; \
...
#define DRIFT_VALIDATE_TERRAIN_SYSTEM(TerrainPtr) \
    if (!TerrainPtr) \
    { \
...
#define DRIFT_VALIDATE_TERRAIN_SYSTEM_RET(TerrainPtr, RetVal) \
    if (!TerrainPtr) \
    { \
        UE_LOG(LogTemp, Log, TEXT("%s: Terrain system is null"), *FString(__FUNCTION__)); \
        return RetVal; \
...
#define DRIFT_VALIDATE_CHUNK_INDEX(ChunkIndex, MaxChunks) \
...
#define DRIFT_VALIDATE_CHUNK_INDEX_RET(ChunkIndex, MaxChunks, RetVal) \
    if (ChunkIndex < 0 || ChunkIndex >= MaxChunks) \
    { \
        UE_LOG(LogTemp, Warning, TEXT("%s: Invalid chunk index %d (max: %d)"), *FString(__FUNCTION__), ChunkIndex, MaxChunks); \
        return RetVal; \
...
#define DRIFT_VALIDATE_SIMULATION_DATA(SimDataPtr) \
...
#define DRIFT_VALIDATE_MASTER_CONTROLLER(MasterPtr) \
    if (!MasterPtr) \
    { \
