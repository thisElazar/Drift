 ...
 *
 * PURPOSE:
 * Player controller for Drift watershed simulator. Multi-mode camera system,
...
class DRIFT_API ATerrainController : public APawn, public IBrushReceiver
{
    GENERATED_BODY()
...
public:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Camera System")
    float FirstPersonHeight = DriftConstants::FIRST_PERSON_HEIGHT;  // 5 feet in cm

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Camera System")
    float TransitionSpeed = DriftConstants::CAMERA_TRANSITION_SPEED;
